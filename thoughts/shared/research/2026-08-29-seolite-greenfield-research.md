...
- **I5 MCP-first parity** — every user-facing capability exists as (a) an MCP tool with namespaced name (`seolite_*`) and (b) a CLI command, backed by the same core engine; (c) a thin REST route exists for the subset that fits the Worker free-tier budget (bounded by I6 — crawl-grade capabilities are explicitly local-only and documented as such). MCP tool descriptions written for agents (high-signal output, concise/detailed response option). Edge: stdio and HTTP transports expose the identical tool set from one codebase.
...
- Adversarial audit round 1 (separate agent, 11 web checks): 2 claims refuted and fixed (CrUX keyless → key required; Open PageRank stale auth/quotas), 1 addition recorded (Programmable Search 2027 shutdown), 8 modality gaps filled (names, tooling, MCP prior art), 6 missing invariant classes added (I12–I17), 4 design-smuggling fixes applied (I7/I9/I10 reworded to requirements; mechanisms deferred to plan). I5 amended after architecture advisory review: REST is a Worker-budget-bounded subset of MCP/CLI parity (crawl stays local).
