...
| I11 | Repo hygiene: public repo, Apache, commit identity Nitish Agarwal `<1592163+nitishagar@users.noreply.github.com>`, NO AI co-author/generated trailers | **CI gate**: commit-identity job rejects any new commit whose author/committer identity deviates or whose message contains `Co-Authored-By:`/`Generated-…` trailers. `dependabot[bot]` is the single allowlisted author (GitHub's official bot, adds no co-author trailers). Existing history verified compliant 2026-08-29 (all 4 commits pass; gate checks new commits only). |
...
| I14 | Concurrency/cancellation/partial failure | Mirrored at CI level: workflow `concurrency` groups cancel superseded branch runs; deploys serialize (`cancel-in-progress: false`); publish treats "already published" as idempotent success (rerun safety, Phase 6). |
