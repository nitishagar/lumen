...
- **A4 Skip-vs-error vocabulary.** A page that is not audited appears in the report as skipped with exactly one machine-readable reason: `robots_disallowed | non_html | oversized | fetch_error | rate_limited | redirect_loop`. Skipped pages carry no issues and contribute no score. Skipped is never reported as an error-severity issue (e.g., robots-denied pages are skipped, not errored).
...
- **A12 Core contract deltas are types-only and additive.** Audit consumes core's Fetcher, config, `AuditRule`/`Severity`/`Issue`/`PageContext`/`PageReport`/`SiteAuditReport`/robots-directive types as declared in ARCHITECTURE. Where audit needs more (optional `Issue.url`, `PageReport.depth/skipped/redirectChain`, `SiteAuditReport.stopReason`, `summary` extras, robots `sitemaps` list), these are optional additive fields — if P1 did not ship them, they land via one types-only PR to core before P2's rule phases merge. Stringly-typed unions (`stopReason`, skip reasons) are declared by audit and stored as `string` in core to avoid a reverse dependency.

## Bounding assumptions (conservative defaults; where a user might have been asked, we fixed the answer)

- Defaults: maxPages 100 (hard clamp 10,000), maxDepth 5, maxDurationMs 120,000, global concurrency 5, per-host minimum interval 250 ms, per-request timeout 15,000 ms, response body cap 2 MiB, redirect-chain cap 10 hops, sitemap caps (10,000 URLs, 10 child sitemaps, 2 MiB), latency threshold 1,500 ms, evidence cap 10 issues per rule per page, frontier seen-set cap 100 × maxPages. Rationale: politeness-first, memory-safe, and all overridable via config.
